...
async def eight_ball(interaction: Interaction, *, question: str):
    ...
    e.set_author(icon_url=interaction.user.display_avatar.url, name=f'🎱 8 Ball')
    await interaction.client.reply(interaction, embed=e)


@app_commands.command()
async def lenny(interaction):
    """( ͡° ͜ʖ ͡°)"""
    ...
    await interaction.client.reply(interaction, content=random.choice(lennys))
...
@app_commands.command()
async def dead(interaction):
    """STOP, STOP HE'S ALREADY DEAD"""
    await interaction.client.reply(interaction, content="https://www.youtube.com/watch?v=mAUY1J8KizU")


@app_commands.command()
async def coin(interaction: Interaction, count: int = 1):
    """Flip a coin"""
    if count > 10000:
        return await interaction.client.reply(interaction, content='Too many coins.')
...
async def urban(interaction: Interaction, *, query: str):
...
@app_commands.command()
async def choose(interaction: Interaction):
...
class Fun(commands.Cog):
    ...
    def __init__(self, bot):
        self.bot = bot
        self.bot.tree.add_command(eight_ball)
        self.bot.tree.add_command(lenny)
        self.bot.tree.add_command(thatsthejoke)
        self.bot.tree.add_command(dead)
        self.bot.tree.add_command(coin)
        self.bot.tree.add_command(urban)
