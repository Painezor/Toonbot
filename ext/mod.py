...
class Mod(commands.Cog):
    ...
    def __init__(self, bot):
        self.bot = bot
        self.bot.tree.add_command(say)
        self.bot.tree.add_command(topic)
        self.bot.tree.add_command(pin)
        self.bot.tree.add_command(rename)
        self.bot.tree.add_command(kick)
        self.bot.tree.add_command(ban)
        self.bot.tree.add_command(unban)
        self.bot.tree.add_command(banlist)
        self.bot.tree.add_command(clean)
        self.bot.tree.add_command(timeout)
        self.bot.tree.add_command(untimeout)
...
async def say(interaction: Interaction, message: str, destination: Optional[discord.TextChannel] = None):
    ...
    if destination.guild.id != interaction.guild.id:
        return await interaction.client.error(interaction, "You cannot send messages to other servers.")

    if not interaction.permissions.manage_messages:
        if not interaction.user.id == interaction.client.owner_id:
            return await interaction.client.error(interaction, "You need manage_messages permissions to do that")

    if len(message) > 2000:
        return await interaction.client.error(interaction, "Message too long. Keep it under 2000.")

    try:
        await destination.send(message)
        await interaction.client.reply(interaction, "Message sent.")
    except discord.Forbidden:
        return await interaction.client.error(interaction, "I can't send messages to that channel.")


@app_commands.command()
@app_commands.describe(new_topic="Type the new topic for this channel..")
async def topic(interaction: Interaction, new_topic: str):
    ...
    await interaction.client.reply(interaction, content=f"{interaction.channel.mention} Topic updated")


@app_commands.command()
@app_commands.describe(message="Type a message to be pinned in this channel.")
async def pin(interaction: Interaction, message: str):
    """Pin a message to the current channel"""
    if not interaction.permissions.manage_channels:
        return await interaction.client.error(interaction, "You need manage_channels permissions to pin a message.")

    message = await interaction.client.reply(interaction, content=message)
    await message.pin()
...
async def rename(interaction: Interaction, member: Member, new_nickname: str):
    ...
    if not interaction.permissions.manage_nicknames:
        ...
        return await interaction.client.error(interaction, msg)

    try:
        await member.edit(nick=new_nickname)
    except discord.Forbidden:
        await interaction.client.error(interaction, "I can't change that member's nickname.")
    except discord.HTTPException:
        await interaction.client.error(interaction, "❔ Member edit failed.")
    else:
        await interaction.client.reply(interaction, content=f"{member.mention} has been renamed.")


@app_commands.command()
@app_commands.describe(member="Pick a user to kick", reason="provide a reason for kicking the user")
async def kick(interaction: Interaction, member: Member, reason: str = "unspecified reason."):
    ...
    try:
        ...
        await interaction.client.error(interaction, f"I can't kick {member.mention}")
    else:
        await interaction.client.reply(interaction, content=f"{member.mention} was kicked.")
...
async def ban(interaction: Interaction, delete_days: Literal[0, 1, 2, 3, 4, 5, 6, 7], member: Optional[Member] = None,
    ...
    if member is not None:
        ...
        try:
            ...
            await interaction.client.reply(interaction, content=message)
        except discord.Forbidden:
            return await interaction.client.error(interaction, f"I can't ban {member.mention}.")
    ...
    if user_id is not None:
        ...
        if delete_days:
            message += f", messages from last {delete_days} day(s) were deleted."
        try:
            await interaction.client.http.ban(user_id, interaction.guild.id)
            await interaction.client.reply(interaction, content=message)
        except discord.HTTPException:
            await interaction.client.reply(interaction, content=f"⚠ Banning failed for UserID# {user_id}.")
...
async def unban(interaction: Interaction, user_id: str):
    ...
    if not interaction.permissions.ban_members:
        return await interaction.client.error(interaction, "You need ban_members permissions to unban someone.")

    try:
        user_id = int(user_id)
    except ValueError:
        return await interaction.client.error(interaction, "Invalid user ID provided.")
    ...
    try:
        ...
        await interaction.client.error(interaction, "I can't unban that user.")
    else:
        target = await interaction.client.fetch_user(user_id)
        await interaction.client.reply(interaction, content=f"{target} was unbanned")
...
async def banlist(interaction):
    ...
    if not interaction.permissions.view_audit_log:
        return await interaction.client.error(interaction, "You need view_audit_log permissions to view the ban list.")
    ...
    view.message = await interaction.client.reply(interaction, content="Fetching banlist...", view=view)
    await view.update()


@app_commands.command()
@app_commands.describe(number="Number of messages to delete.")
async def clean(interaction: Interaction, number: int = None):
...
async def timeout(interaction: Interaction, member: Member,
    ...
    if not interaction.permissions.moderate_members:
        return await interaction.client.error(interaction, "You need moderate_members permissions to time someone out.")
    ...
    try:
        ...
        await interaction.client.error(interaction, "I can't time out that user.")


@app_commands.command()
@app_commands.describe(member="The user to untimeout", reason="reason for ending the timeout")
async def untimeout(interaction: Interaction, member: Member, reason: str = None):
    """End the timeout for a user."""
    if interaction.guild is None:
        return await interaction.client.error(interaction, "This command cannot be used in DMs")

    if not interaction.permissions.moderate_members:
        return await interaction.client.error(interaction, "You need moderate_members permissions to cancel a timeout.")
    ...
    try:
        ...
        await interaction.client.error(interaction, "I can't un-timeout that user.")
